...
- **Duplicación de chrome de proceso entre §1.6, §1.7 y T2 (deuda abierta).** Tres frentes: (a) `AcordeProceso.module.css` y `TriadaProceso.module.css` comparten ~110 líneas literalmente idénticas (panel de pasos, controles, selector de velocidad); (b) los controles ◀/▶/▶▶ + velocidad existen tres veces (`ProcessControls` de T2, `TriadaProceso`, `AcordeProceso`) y **ya divergieron**: los de T2 no tienen `:focus-visible`, o sea un defecto de accesibilidad que existe *porque* hay tres copias, y etiquetan el `<select>` con un patrón distinto; (c) `ProcessControls` ya tiene exactamente la interfaz que devuelve `useProcessAnimation`, así que extraerlo a `shared/` borraría código neto. Prioridad: (b) antes que (a).
- **`AcordeProceso` y `TriadaProceso` viven en `primitives/` pero no son primitivas (deuda de ubicación).** Ambos están atados a una sección concreta: `AcordeProceso` hardcodea `t1.s07.process_eyebrow`, la calidad `'M'` y `BUILDER_17`; `TriadaProceso` hardcodea §1.6. No se pueden montar en otra sección. `primitives/` debería quedar para lo config-driven de verdad (`AcordesBuilder`, `GradosArmonicos`), y el hogar correcto de estos dos sería `modules/tN/components/`. Requiere decisión del dueño: hoy la carpeta dice una cosa y el contenido dice otra.
...
- **`TriadaProceso` tiene el bug de "primer paso mudo" que §1.7 ya corrigió.** Su `lastAudioStep` no se rearma al volver al paso 0, así que `▶▶` → `◀` → `▶▶` ilumina el paso 1 sin sonido. `AcordeProceso` lo cerró rearmando el ref dentro del propio efecto cuando `currentStep === 0`. Aplicar el mismo patrón a `TriadaProceso` para que no diverjan.
...
- **El helper grafía→solfeo está duplicado en tres primitivas más.** La 35ª ola subió `spelledToES` a `data/notes.ts` y migró `AcordesBuilder`, pero `DominanteResolucion.tsx`, `ChordStacks.tsx` e `IntervalRuler.tsx` conservan su propia copia local. Consolidarlas es mecánico; quedó fuera del alcance de la ola.
