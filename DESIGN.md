...
**Última actualización:** 2026-05-06 (después de la 7ª ola — `<NoteToken>` + migración de prosa pedagógica de T1 a `ProseSegment`).
...
- **Densidad de note hues en prosa de T1 tras NoteToken.** La 7ª ola introdujo `<NoteToken>` con color del hue + bg tint al 12% para cada referencia textual a una nota dentro de la prosa pedagógica. En las secciones 03 (Intervalos), 06 (Tríadas) y 07 (Acordes) hay pasos del procedimiento donde >30% del texto del párrafo son tokens de color (ej: `INTERVALOS_PROCEDIMIENTO_PASOS[1]` con 14 nombres de nota seguidos). Decisión 2026-05-06: **aceptable como costo deliberado de la pedagogía** — el reconocimiento visual + auditivo de la identidad cromática es el contenido, no preparación para él. Se documenta como riesgo a re-evaluar cuando llegue feedback de uso real (proyección en clase, lectura prolongada). Mitigaciones disponibles si emerge fricción: (a) bajar bg tint de 12% a 8%, (b) eliminar bg tint dejando solo color del texto, (c) opacity 0.85 al token. **NO ajustar sin reportar primero.**
