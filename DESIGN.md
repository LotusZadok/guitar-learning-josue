...
**The Amber-Means-Sounding Rule (2026-08-09).** Ámbar Pergamino tiene **un solo significado en estado transitorio: "esto está sonando ahora"**. Es el anillo de reproducción de los 6 sitios con highlight progresivo, el `prop playing` de `PlaybackButton`, el inset de `.cellPlaying`, y `.cardActive` de `ProgresionIIVI`. Ningún elemento en reposo puede vestir ámbar para marcar un **rol permanente** (tensión, dominante, pico del arco): si el usuario ve ámbar sin audio corriendo, el sistema le mintió. Los roles se discriminan con tipografía, peso, posición o forma — ver el Corolario abajo. Ámbar sí conserva sus usos **no-transitorios y no-role** ya documentados: `:focus-visible`, y los callouts de §5 (`FExceptionBanner`) donde es el color del callout completo, no un marcador de rol dentro de un set.
...
**The Pitch-Up Rule (2026-08-19, pedido del profesor).** En cualquier diagrama que muestre varias alturas a la vez —árboles de `AcordesBuilder`, stacks de `ChordStacks`— **arriba es más agudo**. La 3M va sobre la 3m, la 4J sobre la 2M, la 5J sobre la 5d, la 7M sobre la 7m sobre la 7d. Es la misma dirección del pentagrama y de la ley "tónica = piso" que ya gobierna el audio. El criterio anterior de los árboles (cercanía a la tónica: menos semitonos más arriba) queda derogado; si una auditoría futura ve un par invertido, es un bug, no una decisión. El eje horizontal es independiente y sigue codificando calidad: menor/disminuida a la izquierda, mayor/justa a la derecha.

**The Chord-Symbol Case Rule (2026-08-19, pedido del profesor).** Los cifrados de acorde (`Gmaj7`, `Gm7`, `Csus4`, `Cdim7`, `Am7♭5`) se renderizan en **IBM Plex Mono**, nunca en Bebas Neue. Motivo: en notación de acordes la caja de la letra **es** semántica — `m` es menor, `M`/`maj` es mayor, y Bebas (todo-caps por diseño de la fuente) convierte `Gmaj7` en `GMAJ7`, que ya no se lee como notación. No es una excepción a la Editorial Trio Rule sino su aplicación literal: "Bebas solo en mayúsculas" significa que un texto cuya minúscula importa no puede ir en Bebas. El test: ¿el string tiene una minúscula que cambia el significado musical? Sí → Plex Mono. Las **letras de nota sueltas** (`G`, `B♭`, `F♯`) no tienen minúscula semántica y siguen en Bebas dentro de los nodos SVG (ver Signature: Chromatic Note Node).
...
- **Notas explicativas = `RuleNote`, no captions centrados (decisión 2026-08-19, pedido del profesor).** Cualquier nota en prosa que interprete el visual de arriba ("la 2ª queda debajo de la tercera menor…", "el sus2 y el sus4 comparten la tónica…") va en `<RuleNote>`: bloque rojizo full-width bajo el texto, left-aligned. El `.caption` mono 12px centrado con `max-width: 560px` **no** es un formato válido para esto; era drift de T3 contra el patrón ya establecido en T1/T2. Un caption de figura genuino (una etiqueta corta que nombra el gráfico, no que lo explica) sigue siendo legítimo, pero hoy no existe ninguno en el proyecto.
...
**Regla (45ª ola):** el chrome editorial de una sección del método — `padding: 48px 60px`, `border-bottom: 1px solid var(--rule)`, entrada `fadeUp 0.6s`, y el paso a `padding: 36px 24px` bajo 900px — vive **sólo** en `shared/sectionChrome.module.css`. Las secciones lo consumen con `composes: section from …`, nunca redeclarando el cuerpo. Mismo mecanismo para `.text`, que aporta **únicamente** `margin-bottom: 16px`: `font-size`, `line-height`, `color` y el cap de 90ch ya los da la regla global `p, li` de `global.css`, y repetirlos en el módulo es ruido que se desincroniza.

Antes de esta ola el bloque vivía redeclarado idéntico en **28** `module.css` de T1/T2/T3, y `.text` en 21. Es el patrón de error que `lessons-learned.md` describe como *"el formato correcto no se busca, se copia del archivo hermano más cercano"*. El test sigue vigente: antes de escribir una clase de presentación en un módulo CSS de sección, grepearla en `src/components` — si aparece en 3+ módulos con el mismo cuerpo, pertenece a `shared/`.

**Quién NO compone:** `ConstructorProgresionesSection` (T4). Su sección es un layout de columnas con `padding: 64px 0` y `gap: 24px` — intención distinta, no una variante del chrome editorial. Extraerla al compartido sería generalizar dos cosas que sólo comparten el nombre de clase.

**`@keyframes fadeUp` se declara dentro del propio módulo compartido, no en `global.css`.** CSS Modules hashea el nombre de la animación cuando se lo referencia desde un módulo, así que un `@keyframes` global nunca resolvía: el bundle tenía 23 referencias `_fadeUp_<hash>_1` distintas contra un único `@keyframes fadeUp` sin hashear, y la animación de entrada de **todas** las secciones estaba muerta desde su introducción. Declarar el keyframe en el mismo módulo alinea los dos hashes. `prefers-reduced-motion` lo sigue neutralizando: el reset global actúa por selector `*`, sin mirar el nombre.
...
**Última actualización:** 2026-08-22 (45ª ola — extracción de `sectionChrome`, auditoría de sobre-ingeniería). Anterior: 2026-08-19 (44ª ola — T4: constructor de progresiones, módulo nuevo). Anterior: 43ª ola — registro voseo → tuteo). Anterior: 42ª ola — §1.7 audio del árbol + pasos re-anclados). Anterior: 41ª ola — §3.8 horizontal + escenarios). Anterior: 40ª ola — §3.6 partido en 3.6.1 tritono + 3.6.2 dominante, ambos sobre el eje horizontal). Anterior: 39ª ola — §3.4 tabla enlazada al árbol, columnas alineadas, §3.5 tritono del vii, §3.6 vii→I, §3.7 copy + color armónico + bloque/arpegio). Anterior: 38ª ola — §3.2 simetría del dim7 como círculo interactivo). Anterior: 36ª–37ª ola — cambios del profesor sobre §2.8, §3.1.1, §3.1.2, §3.3.1, los cinco árboles de nodos y estandarización de notas en T3). Anterior: 2026-07-31 (35ª ola — cambios del profesor sobre §1.6, §1.7, §1.8 y §2.6: tabla de tríadas anclada a la tónica, stepper animado sobre el árbol de acordes, destaque de la nota que suena en el arpegio, y fila de grados reproducible).

### Doctrina activa (decisiones pendientes de diseño)

- **`.text` tiene dos medidas de columna y dos interlineados sin decisión que los separe (45ª ola).** Tras extraer el chrome quedan 21 secciones componiendo el `.text` compartido (hereda `line-height: 1.8` y el cap de 90ch de la regla global `p, li`) y **4** que declaran localmente `line-height: 1.7` + `max-width: 65ch` (`IntroSection` añade además `text-indent: 1.5em`). Las dos formas conviven sin criterio: 65ch es lo que pide la ley tipográfica de Impeccable, 90ch es The 90ch Rule de §3, escrita para la prosa larga del método. **No se unificaron en esta ola a propósito** — elegir una es una decisión tipográfica con impacto visual real en todas las secciones, no una consolidación mecánica. Pendiente: decidir cuál es la medida canónica del cuerpo del método y colapsar la otra.
- **15 hallazgos de `react-hooks` que nadie había visto (45ª ola).** `npm run lint` corría sobre `files: ['**/*.{js,jsx}']` en un repo 100% TypeScript: cero archivos de la app. Corregido a `**/*.{ts,tsx}` con `typescript-eslint`, y ahora cubre los 178 archivos. El preset de TypeScript pasa limpio; los 15 hallazgos son todos de la familia React Compiler: `set-state-in-effect` (6), `refs` (6, escritura de refs en render), `immutability` (1) y `exhaustive-deps` (1). Varios caen sobre patrones **deliberados y documentados en comentario** (`useProgressionPlayback` explica sus deps en 20 líneas; `TriadaProceso` escribe refs en render a propósito). **No se tocaron en esta ola:** son correctness, no sobre-ingeniería, y resolverlos requiere decidir caso por caso si el patrón documentado se defiende o se reescribe. Pendiente: triage de los 15. **No** silenciarlos con `eslint-disable` en bloque — eso reconstruye la puerta falsa que acabamos de quitar.
...
- **Contextos de sonido: tónica-relativos vs identidades absolutas (decisión 2026-06-11).** El principio "tónica = piso" (la tónica activa es la nota más grave; todo orbita ascendiendo desde ella) aplica SOLO a componentes anclados a una tonalidad. Inventario vigente:
  - **Tónica-relativos (piso obligatorio):** `NoteToken` sin prop `octave` (ancla vía `octaveAboveTonic`), `EscalaMayor`, `TensionResolucion`, `GradosArmonicos`, `ProgresionesArmonicas`, `AcordesBuilder`, `TriadaProceso`, `AcordeProceso` (deriva sus notas de `chordSpelled`, que ya calcula octavas ascendentes desde la tónica), `AudioButtons` (escala/tónica), `MasterTriad` (cadena ascendente desde su raíz).
  ...
  - **Identidades absolutas (octava fija 4, SIN piso):** `ChromaticCircleSection` (§1.2), `RelativasMenores`, `SimetriaDim7` (§3.2 — las tres familias de dim7 son absolutas: el dim7 es simétrico, existen tres y no dependen de la tonalidad, así que la tónica global **no** re-ancla el círculo ni transpone sus notas. Lo que sí hace la tónica es **elegir de qué familia se habla**: pertenece a exactamente una, esa queda activa y su nodo se marca con `T`. Un clic del estudiante manda hasta el próximo cambio de tónica), `ChromaticCircleAnimated` (T2, además `playOnClick` nunca se habilita hoy). Presentan las 12 notas como identidades; forzar el piso ahí rompería la presentación neutral del círculo. Si una pasada futura "corrige" estas octavas hacia la tónica, está rompiendo esta decisión.
...
- **`ProgresionIIVI`: el V7 vuelve a llevar color, pero del sistema diatónico (decisión 2026-08-19, pedido del profesor — SUPERA la entrada de 2026-08-09 que sigue abajo).** El profesor pidió "guardar el formato armónico para no olvidar cuáles son tensos (color)". La solución no reabre el conflicto con The Amber-Means-Sounding Rule porque **no usa ámbar**: usa los tokens diatónicos que ya codifican función armónica en §1.4 y §2.6 (`--diatonic-stable` verde, `--diatonic-medium` naranja, `--diatonic-tense` = rojo de marca), con la receta exacta de `GradosArmonicos` (tinte de fondo al 12% + el romano en el token pleno). El ámbar sigue significando "sonando" y vive sólo en `.cardActive`, ahora calificado como `.card[data-role].cardActive` (0,3,0) para ganarle al tinte de rol, con `box-shadow: inset 0 0 0 2px` para leerse sobre un fondo ya tinteado. El cap de 2px en `--paper` del V7 se retiró: era el sustituto del color, y el color volvió. La señal no-cromática se mantiene en el peso 700 del romano.
- **[SUPERADA] `ProgresionIIVI`: el V7 se marca sin color (decisión 2026-08-09).** El pico de tensión pasó de `border-color: color-mix(ámbar 55%, rule)` a **cap superior de 2px en `var(--paper)`** (`box-shadow: inset 0 2px 0` — no `border-top`, para no alterar la altura de la tarjeta) **+ el romano en `--paper` peso 700**, contra `--muted` peso 600 de ii/I. Motivo: el ámbar en reposo colisionaba con `.cardActive`, que usa ámbar para "sonando"; el V7 arrancaba ya vestido del color que en el resto de la app significa reproducción activa. Aplica The Amber-Means-Sounding Rule (§2) y su Corolario de roles. **La entrada histórica de la 34ª ola que dice "el V7 lleva acento de borde ámbar" quedó superada por ésta** — no revertir. Verificado por computed styles en ambos temas: cero ámbar en reposo, ámbar sólo con `.cardActive`, altura idéntica (48px) en las tres tarjetas, cap legible en dark (`#f5f0e8` sobre `--rule #2a2a2a`) y light (`#1a1a1a` sobre `--rule #d0c8c0`).
  - **`DominantesSecundarias` (mismo tratamiento, 2026-08-09):** el cifrado de la dominante pasó de `color-mix(ámbar 70%, paper)` a **`--paper` peso 700**, contra `--paper` peso 600 del destino. La dirección tensión→resolución ya la dan la posición, la flecha `→` y la etiqueta `V/x`; el color era redundante además de colisionar con `.rowActive`. Verificado en ambos temas: cero ámbar en reposo.
  - **`DominanteResolucion` (mismo tratamiento, 2026-08-09):** el anillo del tritono pasó de `stroke: var(--amber)` 2px sólido a **`var(--paper)` 1.5px punteado (`stroke-dasharray="3 3"`)**. El punteado codifica "inestable, quiere moverse" — forma en vez de color, y no compite con ningún otro anillo del componente (el hover rellena el círculo, no lo bordea). Contraste verificado: `#f5f0e8` sobre `#0e0e0e` en dark, `#1a1a1a` sobre `#f5f0e8` en light. El `aria-label` ya decía "· tritono", así que la paridad no-visual no dependía del color.
  - **Pendiente abierto — las flechas de resolución siguen en ámbar.** `DominanteResolucion` (`.arrow` + marker `#dom-arrow`) y su patrón hermano en `TensionResolucion` (§1.5) dibujan las flechas en ámbar estático. Violan la letra de la regla por el mismo motivo, pero **no se tocaron a propósito**: son un patrón compartido entre dos secciones, y cambiar sólo una las desincroniza. Arreglarlo bien es una pasada que toca las dos a la vez y define el neutro (o el rojo diatónico) de las flechas de resolución en todo el método. Decisión pendiente, no olvido.
- **Banco de sonidos intercambiable (decisión 2026-08-09).** El timbre vive detrás de la interfaz `Instrument` en `src/audio/` (`instrument.ts`, `synthInstrument.ts`, `guitarInstrument.ts`, `index.ts`). `useAudioEngine` quedó como dispatcher puro: resuelve la frecuencia (grafía → ASCII → desplazamiento de octava global → `Note.freq`), crea el nodo de voz que `stopAllNotes()` atenúa, y delega el timbre. **Cambiar de banco = reapuntar `instrument` en `src/audio/index.ts`; ningún consumidor cambia.** Banco activo: `guitar-nylon` (samples FluidR3_GM, CC-BY 3.0, ver `public/samples/guitar-nylon/LICENSE.txt`) — 21 muestras C1–C6 cada 3 semitonos, ~364 KB, resampleo máximo de ±1 semitono, precarga al crear el `AudioContext` (primer gesto) y fallback automático a `synthInstrument` si la descarga falla. Nivel calibrado a RMS del banco sintético (`PEAK_GAIN = 1.7`, samples pre-normalizados a pico ~0.12 en todo el rango).
  - Consecuencia aceptada: la guitarra tiene decaimiento **dependiente del registro** (una nota aguda muere mucho más rápido que una grave), a diferencia del sintético que sonaba parejo. Es el comportamiento físico real del instrumento y es deseable pedagógicamente, pero implica que en registros altos las duraciones largas (`NOTE_DURATION`, acordes en bloque) suenan más cortas de lo que dice el parámetro. No es un bug; si molesta, la salida es subir el rango de samples, no alargar el envelope.
  - `playClick`/`playRhythm` (metrónomo) siguen siendo osciladores en el hook y **no** pasan por `Instrument`: son señales de tiempo, no timbre del método.
- **`DominanteResolucion` es el último cifrado que queda en Bebas (2026-08-19).** Sus dos etiquetas SVG son `{grado}7` y la tónica (`G7`, `C`): sin minúscula semántica, así que hoy no viola The Chord-Symbol Case Rule y no se tocó (fuera del alcance del pedido). Queda anotado porque el día que esa primitiva muestre un sufijo con minúscula (`maj7`, `m7`, `sus4`) el bug reaparece silenciosamente. La fix es la misma: `.cifrado` a Plex Mono.
...
- **§1.6: la tabla de tríadas está anclada a la tónica global, la tríada maestra no (decisión 2026-07-31).** La tabla de las 7 tríadas rota desde la letra de la tónica activa, pero el párrafo de la tríada maestra (`t1.s02.master_triad`) sigue enumerando `F A C E G B D F` y el ejemplo del procedimiento (`t1.s02.example`, hoy sin consumidor) sigue en F. El profesor pidió re-anclar **sólo la tabla**. Una auditoría futura que vea la inconsistencia debe consultar esta entrada antes de "arreglarla": el alcance fue una decisión, no un descuido.
- **DE usa solfeo español en la nomenclatura de §1.7.** La copy alemana dice `{{sym}} = {{name}} Dur` con `{{name}}` resolviendo a solfeo (`Fa`, `Re`), no a la letra alemana. Rareza heredada de la traducción original, preservada intacta en la 35ª ola para que el diff de la interpolación fuera quirúrgico. Si se decide corregirla, el cambio correcto es de copy (`{{sym}}-Dur`), no de la interpolación.
- **`AcordesBuilder` tiene modo controlado.** Props `path`/`playingRole`/`onUserPick` (default `undefined`) permiten que un walkthrough maneje el árbol sin duplicar el diagrama; los 4 consumidores de T3 no las pasan y siguen en modo libre. El centinela exportado `PLAYING_ALL` (`'*'`) significa "suena el acorde entero, no atenuar nada". Contrato: si el dueño externo no suelta el control tras `onUserPick`, el clic del estudiante no se refleja visualmente hasta que `path` vuelva a `undefined`.
- **Los `width`/`height` de `AcordesBuilder/configs.ts` tienen una restricción geométrica, no estética.** El anillo de "sonando" (`r = R+6`, stroke 3) se escala 1.12, con alcance real ≈ 35.3px desde el centro del nodo, y el viewBox recorta por defecto. Cada nodo necesita esa holgura contra los cuatro bordes. `BUILDER_17` y `BUILDER_33` pasaron de 440 a 460 de ancho por esto. Si una pasada futura toca `R`, el stroke del anillo o el factor de escala, hay que recalcular las cinco configs.
- **El `<button>` de las celdas de §2.6 no cubre el alto completo del `<td>` (deuda menor, sin verificar en browser).** El botón es un bloque cuyo alto es el de su contenido; el `<td>` lo estira la tabla y `vertical-align: middle` lo centra. En la fila Grado eso deja una franja no clickeable arriba y abajo de las celdas cortas, porque `RomanGlyph` emite tres tamaños distintos (`.romanMajor` 16px, `.romanMinor` 14px, `.romanDim` 12px) y el alto de fila lo fija el mayor. Estimado en 2-4px por borde; la fila Acorde no sufre (`.chordSymbol` es 14px uniforme). El `cursor: pointer` se quitó del `<td>` para que la franja muerta no prometa un objetivo inexistente. Si al verlo proyectado molesta, la fix es `display: flex; align-items: center` en las dos clases de celda más `flex: 1` en el botón — **no** `height: 100%`, que en un hijo de celda de tabla no es confiable entre motores. Ojo: `display: flex` sobre un `<td>` puede sacarlo del modelo de caja de la tabla; verificar la alineación de columnas antes de darlo por bueno.
- **Los controles de proceso tienen una sola copia (resuelto 2026-07-31).** `src/components/shared/ProcessControls/` es la única implementación de la barra ◀ / ▶ / ▶▶ + velocidad; la consumen §1.6, §1.7 y los procesos de T2. Ganó el estilo de T1 (sin `border-radius`, 13px, `.spacer { flex: 1 }`, `<label>` envolvente), decisión del dueño, así que **los controles de §2.3 y §2.4 cambiaron de aspecto**. De paso se corrigió un bug real: los controles de T2 tenían los textos hardcodeados en español y se veían así con la app en alemán. El CSS del panel de pasos vive en `src/components/shared/processChrome.module.css`, consumido importando dos objetos de estilos (`chrome` + `styles`), **no** vía `composes` cruzado: los selectores compuestos como `.stepActive .stepNum` dependen de que ambas clases vengan del mismo módulo, y con `composes` entre archivos esa resolución se vuelve frágil. Efecto secundario aceptado: el `flex-basis` del panel de pasos se unificó en `300px`, así que §1.6 ganó ~20px de base.
- **`.select:focus-visible` de T2 no es paridad con T1, es una mejora neta.** Los gemelos de T1 (`TriadaProceso`, `AcordeProceso`) tienen indicador de foco en sus botones (`.ctrl:focus-visible`) pero **no** en su `<select>` de velocidad. La 35ª ola añadió el de T2. Consecuencia visible: a diferencia de `<button>`, un `<select>` matchea `:focus-visible` también al abrirlo con el mouse en Chromium y Firefox (la heurística lo considera operable por teclado por las flechas), así que el anillo ámbar aparece al hacer clic, no sólo al tabular. Si se decide que eso molesta, la salida no es quitarlo sino igualar los tres controles en la extracción compartida ya registrada.
- **`.chordCellActive[data-harmonic]:hover { color: var(--paper) }` es regla muerta (preexistente).** El único texto de la celda es `.chordSymbol`, que siempre trae su color propio desde `.chordCellActive[data-harmonic="X"] .chordSymbol` en (0,3,0); una declaración sobre el elemento siempre gana a un valor heredado, así que el color de hover nunca llegó al glifo — ni antes ni después de meter el `<button>`. No es regresión de la 35ª ola.
- **El `<select>` de `ProcessControls` (T2) tiene `aria-label` y `aria-labelledby` a la vez.** `aria-labelledby` gana, así que el `aria-label` es código muerto. Preexistente; los gemelos de T1 usan un `<label>` envolvente. Unificar el patrón de etiquetado entra en la extracción compartida ya registrada.
- **`role="button"` sobre un `<td>` saca la celda del árbol de accesibilidad como celda (RESUELTO en la 35ª ola).** Queda la descripción del problema como referencia porque la convención `role="button"` sigue viva en 13 sitios más del proyecto, todos sobre `<g>` de SVG o `<div>` — elementos sin rol implícito que destruir. `<td>` era el único caso donde la convención fallaba. Descripción original: En `GradosArmonicos` las filas Grado y Acorde marcan sus 7 `<td>` como `role="button"`, lo cual **sobrescribe** el `role="cell"` implícito: un usuario de lector de pantalla pierde la navegación por columna, que es justamente lo que hace evidente que `V` y `G` son el mismo acorde nombrado de dos maneras. La convención `role="button"` está en 14 sitios del proyecto, pero en todos los demás es sobre `<g>` de SVG o `<div>`, que no tienen rol implícito que destruir; `<td>` es donde la convención falla. La alternativa es un `<button>` real dentro de la celda, que además **borra** `handleKey`, su `useCallback` y el import de `KeyboardEvent`, y hace innecesario el `aria-hidden` por celda (pasaría al `<tr>`, junto con el `<th scope="row">` que hoy queda anunciado sin celdas cuando la fila está oculta). Es un cambio de convención transversal: requiere decisión del dueño.
- **El `background` de `.cellPlaying` nunca se aplica (deuda cosmética, §2.6).** `.cellPlaying` tiene especificidad (0,1,0) y pierde contra `.gradoCell[data-harmonic]` / `.chordCellActive[data-harmonic]` en (0,2,0). Del estado "sonando" sólo se ve el `box-shadow` inset ámbar, que alcanza para leerlo. No se arregla con orden de fuente (a diferencia del `:hover`, que sí se corrigió calificando con `[data-harmonic]`): haría falta subir la especificidad de la regla.
- **El criterio que separa `primitives/` de `modules/` es el alcance al contenido, no el número de consumidores (decisión 2026-07-31).** Se propuso mover `AcordeProceso` y `TriadaProceso` a `modules/t1/components/` con el argumento de que están atados a una sección. La evidencia lo desmintió: de 18 primitivas, **diez tienen un solo consumidor** (`EscalaMayor`, `ChromaticCircle`, `CirculoDeQuintas`, `TensionResolucion`…), así que "un consumidor" no distingue nada; y `AcordeProceso` era la **única** que referenciaba una clave i18n de sección. `TriadaProceso` no tenía ninguna. La fix correcta no fue mover archivos sino **quitar el alcance**: el eyebrow de `AcordeProceso` pasó a prop desde `AcordesSection`, alineándolo con la regla ya documentada de que el chrome editorial lo provee el consumer. Criterio vigente: **una primitiva puede tener un solo consumidor, pero no puede leer una clave `tN.sXX` ni ningún otro contenido de una sección concreta.** Test de auditoría: `grep -rn "t[0-9]\.s[0-9]" src/components/primitives/*/*.tsx` debe salir vacío.
- **`PATH_BY_STEP`/`PLAYING_BY_STEP` de `AcordeProceso` están acopladas por string a los `role` de `BUILDER_17`.** `BuilderNode.role` está tipado `string`, así que un renombre en `configs.ts` rompe el walkthrough **sin error de compilación**: el árbol simplemente deja de iluminar. Se dejaron como tablas explícitas a propósito (se leen mejor que derivarlas para un recorrido de 4 pasos) y se pasa `config={BUILDER_17}` explícito para que el acoplamiento sea visible.
- **La primitiva `ReglaQuinta` (`src/components/primitives/ReglaQuinta/`) no tiene ningún consumidor.** §1.8 usa `CirculoDeQuintas`; a `ReglaQuinta` sólo la nombra un comentario. Es código muerto candidato a poda, detectado en la 35ª ola y no eliminado por estar fuera de alcance.
- **`.srOnly` vive en `AcordeProceso.module.css` por falta de utilidad global.** No existe ninguna clase de "visualmente oculto" en `global.css` ni en el resto de módulos (se buscó `sr-only`, `srOnly`, `visually-hidden`, `clip-path: inset(50%)`, `clip: rect`). Si aparece una segunda necesidad, extraerla a `global.css` y migrar esta.
- **`data/notes.ts` es el único hogar del helper grafía→solfeo.** `spelledToES` se consolidó ahí en la 35ª ola; las cuatro copias locales (`AcordesBuilder`, `DominanteResolucion`, `ChordStacks`, `IntervalRuler`) desaparecieron. Ojo: el `LETTER_ES` de `src/utils/noteCalculations.ts` **no** es una copia del mismo helper — opera sobre `letter`/`acc` ya separados, con otra firma. No unificarlos.
- **`AcordeProceso` mezcla dos convenciones de i18n.** El eyebrow va por `t()`, pero los labels de los controles y las ~15 cadenas pedagógicas de `buildPaso*` son ternarios `isDe ? ... : ...` inline. No es regresión (venían así de `AcordesSection`), pero deja el archivo a medio camino entre la convención vieja y la que fijó la 34ª ola.
...
- **`--diatonic-tense` no llega a WCAG AA sobre el fondo oscuro: 3.55:1 (medido, 44ª ola).** El token es alias de `--red` (`#c0392b`) y viste los grados tensos (V, vii°) en `GradosArmonicos`, `ProgresionIIVI` y ahora en el constructor de T4. Medido con los computed styles reales: en tema oscuro `stable` 8.81, `medium` 8.16, `mediumTense` 4.52 y **`tense` 3.55** — por debajo del 4.5:1 que exige AA para texto normal (los romanos son 16px/700, y "texto grande" empieza en 18.66px bold). En tema claro los cuatro pasan (mínimo 4.79). **No es deuda de T4:** los mismos números salen en T2 §2.6, que es anterior. Mitigación vigente: el carril tipográfico de `RomanGlyph` (bold recto / regular / itálica) hace que el color no sea el único discriminador, según la doctrina de PRODUCT.md. Resolverlo implica aclarar el rojo de marca en tema oscuro o dar a `--diatonic-tense` un override propio en `:root` — decisión del dueño, afecta las cuatro secciones a la vez.
- **T4: el banco en modo menor no incluye la menor armónica.** El constructor ofrece los 7 grados naturales de la relativa menor y sus tonizaciones. Queda sin decidir si debe incluir chips de `V7` (dominante prestado), `vii°7` (disminuido completo) y `♭III+` (aumentado), que es el vocabulario que el profesor planteó para el módulo. La arquitectura no cambia al añadirlos (el banco consume una lista de grados), pero **el aumentado no tiene rol de color asignado** en el sub-sistema diatónico: `stable`/`medium`/`mediumTense`/`tense` no contemplan esa sonoridad. Requiere decisión del profesor sobre el contenido y del dueño sobre el color.
- **ESLint no lintea ningún archivo de la aplicación.** `eslint.config.js` declara un solo bloque con `files: ['**/*.{js,jsx}']`, y `src/` tiene 110 archivos `.ts`/`.tsx` y cero `.js`/`.jsx`. `npm run lint` solo alcanza bundles vendorizados de `.agents/skills/` y `.claude/skills/` (50 errores preexistentes, ajenos al proyecto); `npx eslint src` falla con exit 2 ("all files matching the glob are ignored"). La red de seguridad estática real es `tsc -b` en strict con `noUnusedLocals`/`noUnusedParameters`, que sí corre. Habilitar linting de TS requiere añadir `typescript-eslint` y elegir un preset: decisión del dueño, fuera del alcance de una ola de feature.
- **El mapa de colores del frontmatter de este documento no son los tokens CSS.** `ambar-pergamino` figura arriba como nombre de color, pero **la variable CSS es `--amber`** (`src/global.css:10`). Un `var(--ambar-pergamino)` no rompe el build, no emite warning y degrada en silencio a `currentcolor`. Pasó en la 44ª ola (lo cazó la revisión, no el compilador). Regla: verificar todo token contra `global.css`, nunca contra el frontmatter.
...
**44ª ola (2026-08-19): T4 — constructor de progresiones (módulo nuevo):**
- ✓ **Módulo T4 con una sección**, el constructor: armadura → par mayor/relativa menor → compás y número de compases → grados sobre las casillas → play. Sandbox puro, sin evaluación ni puntaje ni persistencia (PRODUCT.md). Cuatro primitivas nuevas (`ArmaduraSelector`, `BancoGrados`, `GrillaProgresion`, `TransporteProgresion`), un store sin `persist` y un hook dueño del reloj.
- ✓ **`RomanGlyph` extraído a `shared/`** desde `GradosArmonicos`. Las reglas de color pasaron de selector descendiente (`[data-harmonic] .romanMajor`) a selector del propio elemento (`.romanMajor[data-harmonic]`); verificado sin regresión visual en T2 §2.6, T3 §3.5 y T3 §3.9 con computed styles.
- ✓ **El formato armónico se conserva tal cual** (pedido explícito del profesor): tinte de fondo al 12% del token de rol + romano en el token pleno, la receta de `GradosArmonicos`/`ProgresionIIVI`. Sin tokens nuevos. Los `--note-*` no entran: la grilla habla de grados. El ámbar aparece sólo en la casilla que suena.
- ✓ **Un solo reloj.** Los acordes se agendan enteros al dar play (máx. 48 eventos) con la duración exacta hasta el siguiente, así que el corte sale gratis de la duración y no hay que apagar nada. El cabezal visual lee `ctx.currentTime` en cada frame en vez de correr un temporizador paralelo: no acumula deriva, y si la pestaña se oculta (rAF congelado) se recoloca solo al volver. Cambiar tempo, compás o grilla durante la reproducción **la detiene**: el reloj no se re-negocia en vivo. Verificado en browser.
- ✓ **Los grados sobreviven al cambio de tonalidad.** Verificado: `I C · ii Dm · V G · I C` en Do pasa a `I A · ii Bm · V E · I A` en La sin tocar los romanos. Cambiar de modo sí los reescribe, y avisa antes de vaciar las casillas huérfanas.
- ✓ **Vaciar una casilla dejó de depender del clic derecho.** El borrador original usaba `onContextMenu`, inaccesible en táctil y con teclado — en una app que se usa proyectada y desde el celular. Ahora el gesto es contextual: con un grado armado, tocar coloca; sin nada armado, tocar una casilla ocupada la vacía. El `aria-label` anuncia la acción disponible, no sólo el contenido.
- ✓ **Cada aviso de confirmación vive junto a su disparador.** El de limpiar aparecía arriba de la sección mientras su botón está al final del transporte: quien lo apretaba no lo veía. Medido tras el fix: 26px y 56px del botón respectivo, con el foco en el botón de acción.
- ⚠ **El cabezal visual no pudo verse avanzar en esta sesión.** El panel de vista previa mantiene la pestaña en `visibilityState: "hidden"`, y el navegador congela `requestAnimationFrame` ahí (medido: 0 ticks en 6.3s). La aritmética que lo posiciona (`casillaEn`) sí está verificada exhaustivamente — 30.912 bordes de casilla en los cuatro compases × 40–200 BPM, cero fallos — y la reproducción se sostiene sin detenerse sola (verificado). Lo que falta es ver el borde ámbar recorrer la grilla en un navegador normal.

**42ª ola (2026-08-19): §1.7 el árbol no sonaba + los pasos siguen a la selección:**
- ✓ **El clic sobre un nodo no reproducía nada** (medido interceptando el `AudioContext`: 0 disparos). El audio del árbol salía sólo del `mouseenter`; `pickNode` cambiaba la selección en silencio. Con mouse se oía la nota del hover — salvo en §1.7, donde `onUserPick` → `releaseControl` llamaba `stopAllNotes()` y **cortaba esa misma nota**: el árbol se sentía mudo. Dos cambios: `pickNode` avisa primero al dueño y **después** dispara la nota (el orden importa: al revés, el `stopAllNotes()` del dueño la mataba), y `releaseControl` dejó de llamar `stopAllNotes()`. El debounce de 150ms que ya existía evita el doble golpe hover+clic — verificado: hover + clic sobre el mismo nodo = 1 nota, clic sin hover = 1 nota (antes 0).
- ✓ **Los pasos de §1.7 se re-anclan a la tercera y la quinta elegidas (supera la decisión del 31/7/26 de "construir SIEMPRE la mayor").** `onUserPick` pasa ahora la selección resultante, y `AcordeProceso` deriva de ella el tipo de tríada, los semitonos de cada paso, el nombre del resultado y el recorrido animado. Verificado en Sol: con 3m el paso 2 cuenta hasta 3 s.t. y el resultado dice "G B♭ D · el acorde menor"; sumando 5d, "G B♭ D♭ · el acorde disminuido"; volviendo a 3M la quinta vuelve sola a la justa (3M + 5d no es un camino de `BUILDER_17`).
- ✓ **Efecto colateral deseado en los otros cuatro árboles:** el clic ahora también suena en §3.1.2, §3.2, §3.3 y §3.4, donde antes seleccionaba en silencio.
- ⚠ **La primera interacción tras encender el audio puede salir muda.** El `AudioContext` se crea en el primer gesto y el banco de guitarra precarga en paralelo; hasta que los buffers están, `instrument.playNote` no emite. Se ve en la medición: el primer hover dio 0 disparos y el siguiente 1. No es de esta ola ni del árbol — es del banco de samples — pero conviene saberlo antes de diagnosticar "no suena".

**41ª ola (2026-08-19): §3.8 horizontal + los dos escenarios:**
- ✓ **Las 5 dominantes secundarias pasan de lista apilada a tarjetas lado a lado.** Cada tarjeta lleva el grado destino arriba, la flecha ↑, la dominante y la notación `V/x` abajo — el orden del boceto del profesor (la resolución llega arriba). Entran en una sola fila de 5 × 104px con `flex-wrap` para pantallas angostas.
- ✓ **Bloque nuevo "Los dos escenarios"** (primitiva `EscenariosTonizacion` + helper `tonizacionEscenarios`): selector mayor/menor y los tres grados (2º, 5º, 1º) lado a lado, con clic por acorde y bloque/arpegio para la secuencia. En Do: escenario mayor `II7 D7 · v Gm7 · I7 C7`, escenario menor `ii° Bm7♭5 · v Em7 · i Am7`.
- ✓ **Sin color diatónico en estas tarjetas, a propósito.** El sujeto acá es la **calidad** del acorde (mayor, menor, disminuido), no su función dentro de un arco de tensión; reusar los tokens de §3.7 diría algo que no es. Discriminación por cifrado y romano.
- ⚠ **Lectura pendiente de confirmar por el profesor:** las líneas "escenario mayor: segundo mayor, quinto menor, primero mayor" y "escenario menor: segundo disminuido, quinto menor, primero menor" se interpretaron como **las calidades que toman esos tres grados en cada escenario** — en mayor los tres alterados por la tonización (II→mayor = V/V, V→menor = el ii de IV, I→mayor = V/IV), en menor las calidades naturales de la relativa menor. El escenario menor usa la **relativa** (no la paralela) por consistencia con §3.9 y §3.10. Si el profesor quería otra cosa, lo que cambia es `tonizacionEscenarios` en `noteCalculations.ts`, no la primitiva.

**40ª ola (2026-08-19): §3.6 partido en dos y llevado al eje horizontal:**
- ✓ **Bug de solape corregido.** La etiqueta de cifrado vivía en `y=18` y el anillo punteado del nodo más agudo llegaba a `y=9`: se pisaban en todas las tonalidades. Los apilados bajaron (`v7Y` de `[215,155,95,35]` a `[234,174,114,54]`) antes del rediseño, y el rediseño horizontal lo dejó sin objeto.
- ✓ **§3.6.1 · El tritono**, sección nueva: reusa el mapa de resoluciones de §1.5 (`TensionResolucion`) con una prop `focus="tritono"` que deja sólo los arcos que salen de los dos grados tensos, dibuja el vano de 6 semitonos entre la 4ª y la 7ª, y no toca §1.5 cuando la prop no viene. La idea es del profesor: los dos grados tensos del mapa **son** el tritono de la tonalidad, así que la primitiva ya lo estaba dibujando sin decirlo.
- ✓ **§3.6.2 · El dominante 7**, rediseñado sobre el eje cromático horizontal de §1.4/§1.5** (0–12 semitonos desde la tónica): el acorde de origen va sobre el eje, el I bajo el eje, y los arcos salen de las dos notas del tritono. **Beneficio inesperado: el movimiento contrario ahora se ve literalmente** — la sensible se mueve a la derecha (B en 485 → C en 526) y el 4º grado a la izquierda (F en 239 → E en 198). El apilado vertical anterior no podía mostrarlo con el vii (la flecha salía plana; esa deuda queda cerrada por rediseño, no por parche).
- ✓ **La nota común queda visible por construcción:** el G del V7 y el G del I caen en la misma x (321), uno arriba y otro abajo del eje. Es información real que el diagrama vertical escondía.
- ✓ Numeración: `t3.s36` conserva su clave pero su label pasa a `3.6.2 ·`; la nueva vive en `t3.s361`. Índice lateral y `T3Module` actualizados. Verificado en browser: los dos orígenes (`G7` y `Bm7♭5`) con el tritono en las mismas dos notas, y el índice mostrando 3.6.1 y 3.6.2.

**39ª ola (2026-08-19): cambios del profesor sobre §3.4, §3.5, §3.6 y §3.7:**
- ✓ **§3.4: la tabla de referencia quedó enlazada al árbol.** `AcordesBuilder` gana una salida opcional `onChordChange(cifrado, playing)` que emite por efecto (nunca durante el render); la sección resalta la fila del acorde armado con marco `--paper` + tinte al 7%, y mientras suena con el ámbar de "sonando". Verificado: armar 3m → 5d → 7d enciende exactamente la fila `dim7` con `aria-current`.
- ✓ **Las columnas de los cinco árboles se alinearon a una x por nivel** (§1.7, §3.1.2, §3.2, §3.3, §3.4). El escalonado dentro de un nivel (en §3.4 los cuatro nodos caían en x 125, 140, 160 y 182) era el resto del viejo eje "horizontal = calidad"; la calidad ya la codifican el peso y la itálica del rol, así que no se perdió información. Medido tras el cambio: separación mínima 77px entre centros (necesario 71) y 40px al borde (necesario 36). El ink del SVG ya estaba centrado (16/12px de margen), así que el problema nunca fue el centrado del dibujo sino la raggedness de las columnas.
- ✓ **§3.5: el tritono deja de ser exclusivo del V7.** El párrafo del grado 7 ahora dice que el vii lleva el mismo tritono, entre su raíz (la sensible) y su 5ª disminuida — literalmente las mismas dos notas del V7. Espejado en DE.
- ✓ **§3.6: el visualizador acepta los dos orígenes.** `DominanteResolucion` pasa de tener el V7 cableado a una tabla `ORIGENES` (grados apilados + qué posiciones forman el tritono) con selector de dos chips: `V7 → I` y `vii → I`. Las flechas salen de las posiciones del tritono declaradas en la tabla, no de índices fijos. Verificado en C: `G7` = G B D F con tritono en B/F; `Bm7♭5` = B D F A con tritono en las mismas dos notas.
- ✓ **§3.7: copy del profesor** — "bossa" → "música popular"; el arco pasa a "tensión leve, máxima tensión y resolución". Las etiquetas de las tarjetas se alinearon al mismo vocabulario ("Subdominante · tensión leve", "Dominante · máxima tensión"), que el profesor no pidió explícitamente pero que contradecían al intro dentro de la misma sección.
- ✓ **§3.7: botones de bloque y arpegio** en lugar del botón de texto "Escuchar ii-V-I". Bloque toca las cuatro notas de cada acorde juntas, arpegio las despliega; en ambos casos los tres acordes siguen encadenados. Mueren la prop `playLabel`, la regla `.playBtn` y las claves `t3.s37.play` / `t3.s310.play` en es y de.
- ✓ **§3.7: vuelve el color armónico** (ver la entrada de doctrina, que supera la decisión del 2026-08-09).

**38ª ola (2026-08-19): §3.2 la simetría del dim7 como círculo interactivo:**
- ✓ **Primitiva net-new `SimetriaDim7`**: reemplaza las tres líneas de texto (`C° = E♭° = G♭° = A°` …) por el círculo cromático con el **cuadrado inscrito** de cada familia. Un dim7 son 4 notas cada 3 semitonos, así que es literalmente un cuadrado; las tres familias son el mismo cuadrado rotado un semitono, y eso es lo que el párrafo de la sección explica en palabras. Selector de tres chips + clic en cualquiera de las 12 notas (salta a la familia que la contiene y la hace sonar) + hover/foco para escuchar + bloque/arpegio de la familia activa. Las equivalencias escritas no se perdieron: son el readout bajo el círculo.
- ✓ **Acople con la tónica global, sin transponer.** La pregunta "¿esto no se somete al cambio de tonalidad?" tiene dos mitades: el **contenido** no puede (re-anclar mostraría los mismos tres cuadrados renombrados y borraría la lección de que hay tres y solo tres), pero el **comportamiento** sí — el resto de §3.2 (`ChordStacks`, `AcordesBuilder`) sigue la tónica, y un círculo quieto al lado se lee como desconectado. Solución: la tónica selecciona la familia activa (`useEffect` sobre la tónica cromática) y marca su nodo con `T`. Verificado: tónica E♭ → familia `C°` con la T en E♭; tónica F → familia `D°` con la T en F.
- ✓ **Geometría y audio reusados, no reinventados:** misma construcción que `RelativasMenores` (viewBox 400, R 140, nodos de 24px cada 30°, `NOTE_COLORS` en hover/miembro, octava absoluta 4) y `PlaybackButton` compartido. Las dos familias inactivas se dibujan punteadas en `--rule`; la activa en `--text-body` 2.5. **Sin color por familia**: tres hues indexados por algo que no es una nota violarían la Note-Color Quarantine, así que la discriminación es selección + trazo, no color.
- ✓ **La grafía de cada nodo depende de la familia activa.** Con `C°` activa el nodo de F♯ se lee `G♭`, y con `C♯°` activa vuelve a `F♯`. No es un capricho: es la grafía con que el método escribe cada familia, y es la misma lógica de `chordSpelled` (dentro de un acorde la grafía depende del acorde). Los nodos que no son miembros usan la convención neutra del círculo de §2.8.
- ✓ **§3.3: el ejemplo de resolución sigue la tónica global.** El párrafo estaba clavado en C (`Csus4 → C, F → E`); ahora se arma como `ProseSegment` desde `intervalMemberFromTonic`, así que con tónica G se lee `Gsus4 → G, C → B` y con F♯ mayor `B → A♯`. Las cuatro notas que se mueven son `NoteToken` (hue, audio al pasar, setter de tónica); los **cifrados** van como texto interpolado (`{{sus4}}`, `{{tonic}}`) porque son acordes, no notas sueltas. La clave `resolution` (string plano) se reemplazó por `resolution_parts` (lead/mid/tail) en es y de: el contenido sigue viviendo en i18n, sólo se partió donde entran los tokens. Verificado que con tónica C el texto reproduce la frase original del método carácter por carácter.
- ✓ **`IntervalRuler`: un nodo suena una sola vez (pedido del profesor, §3.3.1).** El clic reproducía el intervalo completo — tónica, pausa de 240ms, nota — para "oír la distancia". Con el mouse eso sonaba tres veces: la nota del hover, la tónica y la nota otra vez. Ahora el hover y el clic comparten el mismo disparo de una nota y el debounce de 150ms absorbe el segundo. **No restaurar el intervalo de dos notas**: fue una decisión, no una regresión. Aplica también a §3.1.1, que usa la misma primitiva. Medido en browser interceptando el `AudioContext`: hover + clic sobre el mismo nodo = 1 nota (antes 3).
- ✓ Verificado en browser: las tres familias (chips, clic en nota y Enter por teclado), readout verbatim del método, cuadrados geométricamente correctos (lados iguales, rotación de 30° entre familias), readout centrado, consola limpia. Typecheck limpio.

**36ª ola (2026-08-19): cambios del profesor sobre §2.8 y §3.3.1 + notas de T3 estandarizadas:**
- ✓ **Las 5 notas de T3 pasaron de `.caption` a `<RuleNote>`** (§3.1.1, §3.1.2, §3.3, §3.3.1, §3.4). T1 y T2 ya usaban el callout rojizo para este tipo de nota; T3 había derivado a un párrafo mono centrado. Cinco reglas `.caption` idénticas murieron con la migración (una por módulo CSS). Doctrina registrada en §5 Cards / Surfaces.
- ✓ **§3.3.1 perdió su tabla** (Intervalo / Semitonos / Forma). El profesor la consideró redundante: los semitonos ya los muestra `IntervalRuler` y la columna "Forma" adelantaba sus2/sus4, que son el tema de §3.3. Con ella murieron `ROWS`, el lookup `table_headers` (es/de) y 5 reglas CSS de tabla. El aire previo a la regla se mantiene porque `IntervalRuler` trae `margin-top: 24px` propio.
- ✓ **Copy: precisión de arriba/abajo en §3.3.1.** "debajo de la tercera" → "debajo de la tercera **menor**"; "justo arriba" → "justo arriba de la tercera **mayor**". La regla dibuja las dos terceras atenuadas, así que la nota tenía que decir cuál flanquea cada intervalo. Espejado en DE (`unter der kleinen Terz` / `über der großen Terz`).
- ✓ **§2.8: `intro_prose` pasó de 2 a 4 párrafos.** Los ejemplos de C mayor y A menor quedaron en renglones propios (cada elemento del array raíz renderiza su `<p>`), con "En" capitalizado y sin el `;`/`.` que colgaban de un salto de línea. El vi grado ahora se lee "de la relativa mayor". Espejado estructuralmente en DE.
- ✓ **Cifrados de `ChordStacks` de Bebas a Plex Mono** (§3.1.2, §3.2, §3.3 — la primitiva es compartida). `GMAJ7`/`GM7` volvieron a leerse `Gmaj7`/`Gm7`; también arregla `sus2`/`sus4`/`dim7`/`m7♭5`, que tenían el mismo problema sin que nadie lo hubiera reportado. 22px Bebas → 16px Plex Mono 600 (Bebas condensada y todo-caps es ópticamente más grande a igual `font-size`). El `<span className={styles.suffix}>` con su `margin-left: 2px` desapareció: en mono el cifrado se escribe pegado, como en una partitura. Regla nueva en §3: The Chord-Symbol Case Rule.
- ✓ **El eje vertical de los árboles se invirtió: ahora "más agudo, más arriba".** Antes codificaba cercanía a la tónica (menos semitonos arriba), así que la 3M quedaba **debajo** de la 3m y la 7M debajo de la 7m — al revés de como se apilan las voces en `ChordStacks` y de como suena. Aplica a las cinco configs (§1.7, §3.1.2, §3.2, §3.3, §3.4). Implementado permutando **sólo las `y` dentro de cada nivel**, no las posiciones completas: así el eje horizontal sigue codificando calidad (menor/dis. a la izquierda) y, como la holgura contra cada borde depende de un solo eje, las clearances quedaron idénticas a las verificadas en la 35ª ola (mínimo medido tras el cambio: 79.4px entre centros, 40px al borde).
- ✓ **El readout del árbol invierte su jerarquía: el cifrado es el titular.** `Cm7` en Plex Mono 24px `--paper`; el nombre en español (`Do menor 7`) baja a alias de 12px `--muted`. Antes era al revés (Bebas 26px para "DO MENOR 7", cifrado de 14px al costado), que además chocaba con The Chord-Symbol Case Rule.
- ✓ **Fuera el readout ámbar de la nota que suena** (`.nowPlaying`), a pedido del profesor. El nodo que suena sigue leyéndose por anillo ámbar + escala 1.12 + hermanos atenuados, así que el arpegio no queda sólo en el audio. Consecuencia aceptada: la región `aria-live` del readout ya no anuncia nota por nota durante el arpegio — anuncia el acorde al cambiar la selección.
- ✓ **Centrado real de los párrafos centrados (bug medido, no cosmético).** `p { max-width: 90ch }` (The 90ch Rule) le da a un párrafo una caja más angosta que su contenedor; con `text-align: center` pero sin `margin-inline: auto`, el texto queda centrado **dentro de una caja pegada a la izquierda**. Medido en el readout del árbol: 84.5px fuera de eje; en el footnote de `CirculoDeQuintas`: 165px. Corregidos ambos. Barrido posterior en DOM: cero párrafos centrados fuera de eje en toda la app.
- ✓ **Centrado vertical del glifo del nodo:** la `y` del `<text>` pasa a depender de la fuente (`y = colored ? 0 : 1`). Medido en browser: bajo `dominant-baseline: middle` la baseline de Bebas 18px cae 6.3px bajo el centro y la ideal para centrar la tinta es 6.5; en Plex Mono 14px cae 3.61 y la ideal es 5. Un solo valor descentraba una de las dos (el glifo de nota quedaba ~1px bajo, ~2px en las grafías con ♯).
- ✓ Typecheck y build limpios. Verificación en browser bloqueada por el gate de contraseña (no ingreso credenciales); el riesgo visual es bajo porque `RuleNote` se reusa verbatim desde T1/T2.
